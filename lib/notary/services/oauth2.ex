  ...
  def get_oauth2_page(req, _stream) do
    ...
    with client when not is_nil(client) <-
      ...
      url =
        case provider do
          :google ->
            "https://accounts.google.com/o/oauth2/auth?client_id=#{client.google_oauth_client_id}&redirect_uri=#{Application.fetch_env!(:notary, :hostname)}/callbacks/google&scope=openid+email+profile&email&response_type=token&state=#{state}"
        end
